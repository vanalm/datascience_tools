...
#         normality = []
#         logging.info("Performing normality tests for each category...")
...
def perform_missing_value_analysis(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if not missing_table.empty:
            ...
            # Visualize missing data matrix
            msno.matrix(df)
            plt.savefig('plots/missing_data_matrix.png')
            plt.close()
...
def analyze_numeric_vs_numeric(df: pd.DataFrame, var1: str, var2: str) -> None:
    ...
    try:
        ...
        logging.info(f"Analysis of '{var1}' vs '{var2}':")
        logging.info(f"Pearson correlation: {pearson_corr:.3f} (p={pearson_p:.3e})")
        logging.info(f"Spearman correlation: {spearman_corr:.3f} (p={spearman_p:.3e})")
    except Exception as e:
        logging.error(f"Error in analyze_numeric_vs_numeric: {e}")
...
def analyze_numeric_vs_categorical(df: pd.DataFrame, numeric_var: str, categorical_var: str) -> None:
    ...
    try:
        ...
        logging.info(f"\n{test_name} Results between '{numeric_var}' and '{categorical_var}':")
        logging.info(f"Statistic: {stat:.4f}, p-value: {p:.4e}")
...
def analyze_categorical_vs_categorical(df: pd.DataFrame, var1: str, var2: str) -> None:
    ...
    try:
        ...
        logging.info(f"\nChi-Squared Test Results between '{var1}' and '{var2}':")
        logging.info(f"Chi-Squared Statistic: {chi2:.4f}, p-value: {p:.4e}, Degrees of Freedom: {dof}")
        ...
        logging.info(f"Cramér's V between '{var1}' and '{var2}': {cramers_v:.3f}")
        ...
        logging.info(f"Association Strength between '{var1}' and '{var2}': {strength}")
    except Exception as e:
        logging.error(f"Error in analyze_categorical_vs_categorical: {e}")
